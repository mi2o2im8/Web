<?php
    ...
    $result_table= mysqli_query($db, $sql); //selet의 조건에 따른 경과표를 리턴해줌
    ...
    if($result_table){
        ...
        // 반복문을 통해.. 총 레코드의 수 만큼 한줄식 데이터를 읽어와서 사용자에게 보여주기(응답. echo)
        for($i=0; $i<$row_num; $i+=1){
            $row=mysqli_fetch_array($result_table, MYSQLI_ASSOC); //결과표에서 하나줄 데이터를 (연관)배열로 받기
            
            //한줄에서 각 칸(테이블의 colim이름)들의 값들을 뽑아오기
            $no= $row['no'];
            $name= $row['name'];
            $title= $row['title'];
            $message= $row['message'];
            ...
            if($file_path){ //첨부 이미지 파일이 있다면
                echo "<img src='$file_path' alt='첨부이미지' width='200px'>";
            }
            echo "<hr>";
