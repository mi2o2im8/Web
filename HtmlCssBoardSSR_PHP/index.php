<?php
    ...
    // 데이터베이스의 web_board테이블의 게시글 리스트를 읽어와서 아래 html에 표시...

    //MySQL DBMS과 연결하여 데이터들 가져오기
    $db= mysqli_connect('localhost', 'endoyo', 'als2d3f4!', 'endoyo'); //DB주소, DB접속아이디, DB접속비번, DB명
    mysqli_query($db, 'set names utf8'); //한글깨짐 방지

    // 원하는 쿼리문 수행(SQL언어 - web_board테이블의 모든 데이터들 번호기준 내림차순으로..[최신글 순으로])
    $sql= "SELECT * FROM web_board ORDER BY no DESC"; //넘거칸에 대해서 디샌딩 내림차순으로.
    ...
    // CSR 구조에서는 php의 역할은  ..이 게시글 데이턷ㄹ만 사용자에게 응답.
    //echo $board_list;
    // echo json_encode($board_list); //대량의 데이터를 json 형식으로 응답

?>

<!-- php는 결국 사용자가 보는 페이지를 echo 하기에 html태그문을 그냥 쓰면 자동 echo  (뭘로쓰나 똑같)-->
    ...
    <div class="board_wrap">
        ...
        <div class="board_list_wrap">
            ...
            <table class="board_list">
                ...
                <!-- 2) 게시글 데이터를 표시영역(php나 JS 반복문으로 구현예정) ~ 우선 디자인 목적으로 가짜 데이터 구성 -->
            <?php 
